<template>
  <div>
    ...
    <v-text-field  v-model="searchValue" clearable label="Pesquisar" prepend-icon="mdi-magnify" dense>
      <v-tooltip activator="parent" location="bottom">Digite o termo da pesquisa e tecle Enter...
      </v-tooltip>
    </v-text-field>

    <EasyDataTable buttons-pagination :headers="headers" :items="items" :search-field="searchField" :search-value="searchValue" theme-color="#f48225" table-class-name="customize-table" alternating>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Descrição', value: 'name', sortable: true },
      { text: 'Valor', value: 'price', sortable: true },
      { text: 'Categoria', value: 'category', sortable: true },
      { text: 'Avaliação', value: 'rating', sortable: true },
    ],
...
.customize-table {
  ...
  --easy-table-body-row-hover-background-color: #4DB6AC !important; 
}
